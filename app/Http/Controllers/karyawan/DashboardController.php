...
class DashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $total_absen_masuk              = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'IN')->count();
        $total_absen_masuk_tepat        = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'IN')->where('status', 'VALID')->count();
        $total_absen_masuk_tidak_tepat  = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'IN')->where('status', 'TIDAK VALID')->count();

        $total_absen_pulang             = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'OUT')->count();
        $total_absen_pulang_tepat       = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'OUT')->where('status', 'VALID')->count();
        $total_absen_pulang_tidak_tepat = Absensi::where('id_karyawan', Auth::user()->id)->where('jenis_absen', 'OUT')->where('status', 'TIDAK VALID')->count();

        $izin_sakit         = Izin::where('id_karyawan', Auth::user()->id)->where('jenis_izin', 'SAKIT')->count();
        $izin_cuti_pending  = Izin::where('id_karyawan', Auth::user()->id)->where('jenis_izin', 'CUTI')->where('approve', 0)->count();
        $izin_cuti_approved = Izin::where('id_karyawan', Auth::user()->id)->where('jenis_izin', 'CUTI')->where('approve', 1)->count();
        $izin_cuti_rejected = Izin::where('id_karyawan', Auth::user()->id)->where('jenis_izin', 'CUTI')->where('approve', 2)->count();
        
        return view('karyawan.dashboard.dashboard')->with(compact('total_absen_masuk', 'total_absen_pulang', 'total_absen_masuk_tepat', 'total_absen_masuk_tidak_tepat', 'total_absen_pulang_tepat', 'total_absen_pulang_tidak_tepat', 'izin_sakit', 'izin_cuti_pending', 'izin_cuti_approved', 'izin_cuti_rejected'));
    }
    ...
    public function profil()
    {
        ...
        return view('karyawan.profile.index')->with(compact('day', 'month', 'year'));
    }
